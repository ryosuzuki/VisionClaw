...
async def entrypoint(ctx: JobContext):
    ...
    logger.info(
        "session start: user=%s engine=%s action_backend=%s",
        user_id,
        engine,
        action_backend,
    )
        ...
        def _handle_openclaw_response(reader: rtc.TextStreamReader, from_identity: str) -> None:
            async def read_response() -> None:
                if from_identity != user_id:
                    logger.warning("ignored OpenClaw response from unexpected participant")
                    return
                try:
                    resolve_openclaw_response(userdata, await reader.read_all())
                except Exception as exc:
                    logger.warning("invalid OpenClaw response: %s", type(exc).__name__)
